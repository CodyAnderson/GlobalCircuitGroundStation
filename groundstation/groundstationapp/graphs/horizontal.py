...
def horizontal(getParams):
	...
	ordered_fastmeasurements = models.FastMeasurement.objects.select_related('IridiumData').select_related('Packet').annotate(packet_transmit_time=F('global_id__global_id__transmit_time')).annotate(sample_delay=ExpressionWrapper(F('sub_id')*timedelta(seconds=5), output_field=DurationField())).annotate(sample_time=ExpressionWrapper(F('packet_transmit_time')+F('sample_delay'), output_field=DateTimeField())).filter(packet_transmit_time__gte=minTime).filter(packet_transmit_time__lte=maxTime).order_by('global_id', 'sub_id')
